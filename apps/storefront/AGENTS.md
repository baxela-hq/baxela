...
- Auth session: `context/auth-context.tsx` stores the Sanctum token + user in localStorage (keys in `lib/auth-storage.ts`), validates on mount via `GET /auth/user/account/me`, and exposes `useAuth()` (`status/user/token/signIn/signOut`). The backend has no signout endpoint — signing out only drops the local session. Any token-bearing 401 from `apiFetch` (including the mount validation) clears the stored session and hard-redirects via `window.location.assign` to `/{locale}/login?next=<locale-less current path>&session_expired=1`; the login page toasts the expiry and strips the flag. Guest flows (sign-in, cart, OTP) send no token and never trigger it.
- Guest cart: `lib/cart/client.ts` keeps a CSPRNG-UUID cart token in localStorage (`baxela_cart_token`) and `cartApi(token)` picks the audience — `/cart/user/*` with the bearer token when signed in, `/cart/public/*` with `X-Cart-Token` for guests. Cart pages never redirect to login; only checkout guards (redirects to `/login?next=/checkout`). `signIn` sends the cart token so the backend merges the guest cart into the account cart, then clears the local token only after success.
...
- Data sources note: site menus (header nav, footer link columns, mega-menu columns) come from the public Menu API (`lib/api/site.ts` `fetchMenu` + `lib/menu.ts` helpers); CMS pages render from the Content API under `/pages/[slug]`. Product, category, option (size/color), comment, cart, address, shipping and order data also all come from the API.
