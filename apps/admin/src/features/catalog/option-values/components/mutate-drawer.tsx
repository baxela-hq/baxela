...
import { useSaveOptionValue } from '../hooks/use-option-value-mutations'
...
export function MutateDrawer({
  ...
  const saveOptionValue = useSaveOptionValue()

  const onSubmit = (data: OptionValueForm) => {
    saveOptionValue.mutate(
      { optionId, id: currentRow?.id?.toString(), data },
      {
        onSuccess: () => {
          onOpenChange(false)
          form.reset()
