...
import { createOptionValue, updateOptionValue } from '../api/option-values.api.ts'
...
export function MutateDrawer({
  ...
  const { tLabel, tHelpText } = useAppTranslation(Locales.OPTION_VALUE)
  ...
  const form = useForm<OptionValueForm>({
  ...
  const onSubmit = async (data: OptionValueForm) => {
    try {
      if (isUpdate){
        await updateOptionValue(optionId, currentRow?.id.toString(), data)
      } else {
        await createOptionValue(optionId, data)
      }
    ...
    >
      <SheetContent className='flex flex-col'>
        ...
        <SheetFooter className='gap-2'>
          ...
          <Button form='option-values-form' type='submit'>
