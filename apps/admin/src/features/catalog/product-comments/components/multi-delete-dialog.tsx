...
import { useBulkDeleteProductComments } from '../hooks/use-product-comment-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteProductComments = useBulkDeleteProductComments()

  const handleDelete = () => {
    if (value.trim() !== CONFIRM_WORD) {
      toast.error(
        t('dialog.bulk_delete.type_to_confirm', { word: CONFIRM_WORD })
      )
      return
    }
    ...
    toast.promise(
      bulkDeleteProductComments.mutateAsync(
        selectedRows.map((row) => row.original as ProductComment)
      ),
      {
        ...
        success: () => {
          ...
          return t('dialog.bulk_delete.deleted_items_x', {
            ...
            name:
              selectedRows.length > 1 ? t('shared.items') : t('shared.item'),
          })
        },
      ...
      desc={
        <div className='space-y-4'>
          ...
          <Label className='my-4 flex flex-col items-start gap-1.5'>
            <span className=''>
              {t('dialog.bulk_delete.confirm_by_typing')} "{CONFIRM_WORD}":
            </span>
            <Input
              value={value}
              onChange={(e) => setValue(e.target.value)}
              placeholder={t('dialog.bulk_delete.type_to_confirm', {
                word: CONFIRM_WORD,
              })}
            />
          </Label>
