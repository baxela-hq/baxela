...
import { Trash2, UserPen, ListFilter } from 'lucide-react'
...
type DataTableRowActionsProps = {
  row: Row<AttributeGroup>
}

export function DataTableRowActions({ row }: DataTableRowActionsProps) {
  const { setOpen, setCurrentRow } = useAttributeGroups()
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align='end' className='w-[160px]'>
          ...
          <DropdownMenuItem
            onClick={() => {
              navigate({
                to: FeatureRoutes.ATTRIBUTES,
                search: { 'filter[group_id]': [String(row.original.id)] },
              })
            }}
          >
            {tLabel('attributes')}
