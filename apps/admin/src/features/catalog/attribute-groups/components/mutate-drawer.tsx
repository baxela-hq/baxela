...
import { useSaveAttributeGroup } from '../hooks/use-attribute-group-mutations'
...
export function MutateDrawer({
  ...
  const { tLabel, tHelpText } = useAppTranslation(Locales.ATTRIBUTE_GROUP)
  ...
  const form = useForm<AttributeGroupForm>({
  ...
  const saveAttributeGroup = useSaveAttributeGroup()

  const onSubmit = (data: AttributeGroupForm) => {
    saveAttributeGroup.mutate(
    ...
    >
      <SheetContent className='flex flex-col'>
        ...
        <SheetFooter className='gap-2'>
          <Button form='attribute-groups-form' type='submit'>
