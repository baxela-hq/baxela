...
import { createAttributeGroup, updateAttributeGroup } from '../api/attribute-groups.api'
...
export function MutateDrawer({
  ...
  const onSubmit = async (data: AttributeGroupForm) => {
    try {
      if (isUpdate){
        await updateAttributeGroup(currentRow?.id.toString(), data)
      } else {
