...
import { useBulkDeleteContactMessages } from '../hooks/use-contact-message-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteContactMessages = useBulkDeleteContactMessages()
  ...
  const handleDelete = () => {
    ...
    toast.promise(
      bulkDeleteContactMessages.mutateAsync(
        selectedRows.map((row) => row.original as ContactMessage)
      ),
      {
