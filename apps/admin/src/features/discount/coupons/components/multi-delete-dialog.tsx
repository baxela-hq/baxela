...
import { useBulkDeleteCoupons } from '../hooks/use-coupon-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteCoupons = useBulkDeleteCoupons()
  ...
  const handleDelete = () => {
    ...
    toast.promise(
      bulkDeleteCoupons.mutateAsync(
        selectedRows.map((row) => row.original as Coupon)
      ),
      {
