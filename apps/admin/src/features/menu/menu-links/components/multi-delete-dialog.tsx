...
import { useBulkDeleteMenuLinks } from '../hooks/use-menu-link-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteMenuLinks = useBulkDeleteMenuLinks()
  ...
  const handleDelete = () => {
    ...
    toast.promise(
      bulkDeleteMenuLinks.mutateAsync(
        selectedRows.map((row) => row.original as MenuLink)
      ),
      {
