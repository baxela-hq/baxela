...
import { useBulkDeleteMenus } from '../hooks/use-menu-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteMenus = useBulkDeleteMenus()
  ...
  const handleDelete = () => {
    ...
    toast.promise(
      bulkDeleteMenus.mutateAsync(
        selectedRows.map((row) => row.original as Menu)
      ),
      {
