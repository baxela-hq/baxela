...
import { useBulkDeleteMethods } from '../hooks/use-method-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteMethods = useBulkDeleteMethods()
  ...
  const handleDelete = () => {
    ...
    toast.promise(
      bulkDeleteMethods.mutateAsync(
        selectedRows.map((row) => row.original as Method)
      ),
      {
