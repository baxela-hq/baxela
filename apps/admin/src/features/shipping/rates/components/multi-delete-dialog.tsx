...
import { useBulkDeleteRates } from '../hooks/use-rate-mutations'
...
export function MultiDeleteDialog<TData>({
  ...
  const bulkDeleteRates = useBulkDeleteRates()
  ...
  const handleDelete = () => {
    ...
    toast.promise(
      bulkDeleteRates.mutateAsync(
        selectedRows.map((row) => row.original as Rate)
      ),
      {
