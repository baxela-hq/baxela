...
export function DataTableBulkActions<TData>({
  ...
  entityName = entityName ?? {
    singular: t('shared.item'),
    plural: t('shared.items'),
  } as EntityType;
  ...
  useEffect(() => {
    ...
    if (selectedCount > 0) {
      const message = `${selectedCount} ${selectedCount > 1 ? entityName.plural : entityName.singular} ${t('dialog.bulk.selected')}. ${t('dialog.bulk_delete.toolbar_available')}.`
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        role='toolbar'
        aria-label={t('dialog.bulk.toolbar-aria', {
          selectedCount: selectedCount,
          name: selectedCount > 1 ? entityName.plural : entityName.singular }
        )}
        ...
        >
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={`${t('dialog.bulk.clear-selection')} (Escape)`}
                title={`${t('dialog.bulk.clear-selection')} (Escape)`}
              >
                <X />
                <span className='sr-only'>{t('dialog.bulk.clear-selection')} (Escape)</span>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>{t('dialog.bulk.clear-selection')} (Escape)</p>
            </TooltipContent>
          </Tooltip>
          ...
          >
            <Badge
              ...
              aria-label={`${selectedCount} ${t('dialog.bulk.selected')}`}
            >
              {selectedCount}
            </Badge>{' '}
            <span className='hidden sm:inline'>
              {selectedCount > 1 ? entityName.plural : entityName.singular}
            </span>{' '}
            {t('dialog.bulk.selected')}
          </div>
