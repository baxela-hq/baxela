...
export type sortingCfg = { key: 'sort' }
...
export function useTableUrlState(
  ...
  const sortKey = sortingCfg?.key ?? 'sort';
  ...
  const initialSorting: SortingState = useMemo(() => {
    ...
    if (typeof rawSort === 'string' && rawSort.trim() !== '') {
      ...
      const id = rawSort.startsWith('-') ? rawSort.substring(1) : rawSort;
  ...
  const onSortingChange: OnChangeFn<SortingState> = useCallback((updater) => {
    ...
    let sortParamValue: string | undefined = undefined;
    if (next.length > 0) {
      const { id, desc } = next[0];
      sortParamValue = `${desc ? '-' : ''}${id}`;
    }

    navigate({
      search: (prev) => ({
        ...(prev as SearchRecord),
        [sortKey]: sortParamValue,
      }),
    });
  }, [navigate, sortKey, sorting]);
